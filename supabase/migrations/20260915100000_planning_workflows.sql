-- Complete planning workflows: commercial imports, conflicts, deterministic weekly plans,
-- reversible changesets and daily snapshots. Additive; existing data is preserved.
...
create unique index if not exists coverage_inputs_identity_idx on public.coverage_inputs
  nulls not distinct (user_id,input_type,cycle_start,microarea,product_id);
...
begin
  ...
  for r in select * from public.coverage_import_staging where import_id=p_import_id and user_id=uid and classification='certo' and applied_at is null order by row_number for update loop
    ...
    values(uid,input_kind,r.normalized_data->>'cycle_start',r.normalized_data->>'cycle_end',upper(r.normalized_data->>'microarea'),product,
    ...
    on conflict (user_id,input_type,cycle_start,microarea,product_id) do update set
      cycle_end=excluded.cycle_end, planned_days=coalesce(excluded.planned_days,coverage_inputs.planned_days),
      target_value=coalesce(excluded.target_value,coverage_inputs.target_value), actual_value=coalesce(excluded.actual_value,coverage_inputs.actual_value),
      source_import_id=excluded.source_import_id, metadata=excluded.metadata;
    update public.coverage_import_staging set applied_at=now() where id=r.id;
    applied := applied+1;
  end loop;
  update public.imports set status=case when exists(select 1 from public.coverage_import_staging where import_id=p_import_id and classification='incompleto_ambiguo') then 'da_revisionare' else 'applicato' end,
...
-- Supabase Cron setup is idempotent. The job calls a server-only function and contains no secret.
